...
imgpath =".\images\";
% 创建输出目录
outputpath_bg = '.\result\background\';
outputpath_target = '.\result\target\';
...
for l = 1:patch_num
    disp(['Processing patch ' num2str(l) '...']);
    ...
    for i = 1:patch_frames
        ...
        % Save the extracted background and target images
        bgFilename = [outputpath_bg 'background_' num2str(patch_frames*(l-1)+i) '.bmp'];
        targetFilename = [outputpath_target 'target_' num2str(patch_frames*(l-1)+i) '.bmp'];
        imwrite(A, bgFilename);
        imwrite(E, targetFilename);
    end
end
...
disp(['Total time: ' num2str(time_all) ' seconds']);
disp(['Time per image: ' num2str(time_per_img) ' seconds']);